...
# ------------------------------------------------------------
# Rainfall ecology model
# ------------------------------------------------------------

class SupervisedEcoRainfallEncoder(nn.Module):
    def __init__(
        self,
        embedding_dim=512,
        n_sat=64,
        ...
        self.sat_head = nn.Linear(embedding_dim, n_sat)
        self.habitat_head = nn.Linear(embedding_dim, n_habitats)
        self.rain_head = nn.Linear(embedding_dim, 1)
...
def list_audio_files(root):
    ...
    for ext in exts:
        ...
        files.extend(Path(root).rglob(f"*{ext.upper()}"))
...
def main():
    ...
    n_sat = len(ckpt.get("sat_cols", []))
    if n_sat == 0:
        n_sat = 64

    n_habitats = len(ckpt.get("habitat_classes", []))
    if n_habitats == 0:
        n_habitats = 4

    print("n_sat:", n_sat)
    print("n_habitats:", n_habitats)

    model = SupervisedEcoRainfallEncoder(
