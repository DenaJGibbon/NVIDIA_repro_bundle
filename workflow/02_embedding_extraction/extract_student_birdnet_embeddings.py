...
def main():
    ...
    student_dim = ckpt.get(
        "student_dim",
        ckpt_args.get("student_dim", 512)
    )
    teacher_dim = ckpt.get(
        "teacher_dim",
        1024
    )
    ...
    with torch.no_grad():
        ...
        for specs, paths in pbar:
            ...
            student_emb, pred_teacher = model(specs)

            emb = student_emb.cpu().numpy()
